...
| 文件 | 改动 |
| --- | --- |
| [`caddy/Caddyfile`](../caddy/Caddyfile) | `:443` + `tls` 指定手动证书 + 反代到 `openclaw-gateway:{$OPENCLAW_GATEWAY_PORT:18789}` |
| [`caddy/generate-cert.sh`](../caddy/generate-cert.sh) | 新增：按 `CADDY_IP` 生成自签证书（SAN 含 IP），输出 `caddy.crt` / `caddy.key` |
| [`docker-compose.yml`](../docker-compose.yml) | 明文网关端口改为绑 `127.0.0.1`；新增 `caddy` service（只暴露 443，挂载证书） |
